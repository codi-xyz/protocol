...
describe('ProtocolMeta', () => {
  ...
  describe('ProtocolMetaParser.fromInitiator', () => {
    ...
    it('should create protocol meta from initiator with custom prefix', () => {
      ...
      expect(result).toEqual({
        ...
        prefix: 'CUSTOM',
        initiator: 'ABC123',
        id: CodeGenerator.deriveCodeHash(initiator, 'CUSTOM'),
        iss: ISSUER
      });
    });
  ...
  describe('ProtocolMetaParser.validateCode', () => {
    it('should validate correct protocol meta', () => {
      ...
      const meta = ProtocolMetaParser.fromInitiator(initiator, ISSUER, prefix);
  ...
  describe('ProtocolMetaParser.validateMetaFromString', () => {
    it('should validate correct meta string', () => {
      const initiator = 'ABC123';
      const meta = ProtocolMetaParser.fromInitiator(initiator, ISSUER);
  ...
  describe('Edge cases', () => {
    ...
    it('should handle empty prefix', () => {
      const result = ProtocolMetaParser.fromInitiator('ABC123', ISSUER, '');
      expect(result.prefix).toBe('');
      expect(result.id).toBe(CodeGenerator.deriveCodeHash('ABC123', ''));
      expect(result.iss).toBe(ISSUER);
    });
